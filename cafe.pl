...
#If annotation file is not provided
	...
	system("prodigal -a $infile\_CAFE.faa -f sco -i $in_filename -o $name\_CAFE_sco_file.txt -q"); #bartonella =.fna file
...
for ($i=0; $i< $ncondcounter; $i++){
	...
	if ($nca3[$i]==121212){
		if ($nca2[$i]-$nca1[$i]>=8000){
			...
			print OUTFILE1 "$nul\t$nca1[$i]\t$nca2[$i]\t$length\n";
		}
	}
