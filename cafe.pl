...
if ($phylo==1){
	...
	system ("blastp -db faa_database -query $infile\_CAFE.faa -outfmt \"6 qseqid sseqid stittle salltitles qcovs pident\" -out $infile\_blast_output -num_threads 40");
	$blastf="$infile\_blast_output";
	open(BL, $blastf);
	open (BLOUT,">$infile\_phy");
...
#$clus=$sc_clus $phylo=$phylovalue 

$clusno=max(@cond_arr3); #total number of clusters
...
for ($j=1; $j<($clusno+1);$j++){ 
	...
	if (($count_aberrant[$j]/$count_gene[$j])>=0.75){ # if 50% of the genes in cluster are aberrant
		$abb[$j]=1;
	}
	else {$abb[$j]=0;}
}
...
for ($i=0; $i<scalar(@cond_arr3); $i++){
	...
	#print "$oriclusid[$i] $cond_arr3[$i]\n";
}

$ncondcounter=0;

for ($i=0; $i< scalar(@cond_arr3); $i++){		#Need to condense
...
for ($i=0; $i< $ncondcounter; $i++){
	...
	if ($nca3[$i]==121212){
		if ($nca2[$i]-$nca1[$i]>=8000){
			...
			print OUTFILE1 "$nca1[$i]\t$nca2[$i]\t$length\n";
		}
	}
