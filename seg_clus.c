...
for (i=1;i<sg1;i++){ // for all segments
	...
	//If segments do not merge
		...
		if(i+1==sg1){ //for last segment
			...
			printf("fgsi %d final_groups_start[i] %d final_groups_end[i] %d\n",fgs-1,final_groups_start[fgs-1], final_groups_end[fgs-1] );
		...
		printf("fgsi %d final_groups_start[i] %d final_groups_end[i] %d\n",fgs,final_groups_start[fgs-1], final_groups_end[fgs-1] );
	    }
...
for(i=1;i<fgs+1;i++){
			...
			final_s1_end[sg1]=final_groups_end[i];
			sg1+=1;
		  }
count=0;count1=0;
//printf ("sg1 is %d\n",sg1);
...
for (i=1;i<sg1;i++){
	...
	if (smax<thres1){
		...
		if(i+1==sg1){
			...
			switcarray[len]=1;
			//fprintf(output2,"fgsi %d final_groups_start[i] %d final_groups_end[i] %d\n",fgs,final_groups_start[fgs], final_groups_end[fgs] );
			printf("2final_groups_start[i] %d final_groups_end[i] %d\n",final_groups_start[fgs-1], final_groups_end[fgs-1] );
			total_cluster_length[fgs]=final_groups_end[fgs]-final_groups_start[fgs];
			fgs+=1;
		...
		//fprintf(output2,"1final_groups_start[i] %d final_groups_end[i] %d\n",final_groups_start[fgs-1], final_groups_end[fgs-1] );
		printf("1final_groups_start[%d] %d final_groups_end[%d] %d\n",fgs-1,final_groups_start[fgs-1], fgs-1,final_groups_end[fgs-1] );
	    }
